...
function renderPortfolio(data) {
  document.getElementById('tb-port').textContent = data.strategies?.length ?? '—';
  if(!data.strategies?.length) return `<div class="empty"><div class="icon">💼</div><div class="title">NO TRADE DATA YET</div><div class="sub">Portfolio simulation runs after entry signals are recorded</div></div>`;
  ...
  <div class="summary-bar" style="margin-bottom:14px">
    <div class="sbar-item"><div class="sbar-v" style="color:var(--cyan)">${data.total_trades}</div><div class="sbar-l">Trades</div></div>
    <div class="sbar-item"><div class="sbar-v" style="color:var(--amber)">${data.bet_sol} SOL</div><div class="sbar-l">Bet Size</div></div>
    <div class="sbar-item"><div class="sbar-v" style="color:var(--dim)">${data.total_deployed} SOL</div><div class="sbar-l">Deployed</div></div>
    <div class="sbar-item"><div class="sbar-v" style="color:var(--green)">${best.name}</div><div class="sbar-l">Best Strategy</div></div>
  </div>`;
  const rows = data.strategies.map((s,i)=>{
    const isBest  = i===0;
    const isWorst = i===data.strategies.length-1;
    const pnlCls  = s.total_pnl>0?'pos':s.total_pnl<0?'neg':'zero';
    const pnlSign = s.total_pnl>0?'+':'';
    const maxAbs  = Math.max(...data.strategies.map(x=>Math.abs(x.total_pnl)),0.001);
    const barPct  = Math.abs(s.total_pnl)/maxAbs*100;
    const barCol  = s.total_pnl>0?'var(--green)':'var(--red)';
    return `
    <div class="port-strat${isBest?' best':isWorst?' worst':''}">
      <div class="port-top">
        <div class="port-name">${isBest?'🏆 ':''}${isWorst?'📉 ':''}${s.name}</div>
        <div class="port-pnl ${pnlCls}">${pnlSign}${s.total_pnl.toFixed(4)} SOL</div>
      </div>
      <div class="port-desc">${s.desc}</div>
      <div class="port-meta">
        <span>Wins: <span class="hl" style="color:var(--green)">${s.win_count}</span></span>
        <span>Losses: <span class="hl" style="color:var(--red)">${s.loss_count}</span></span>
        <span>Win Rate: <span class="hl">${s.win_rate}%</span></span>
        <span>Avg P&L: <span class="hl">${s.avg_pnl>0?'+':''}${s.avg_pnl.toFixed(4)} SOL</span></span>
        <span>Best: <span class="hl" style="color:var(--green)">+${s.best_trade.toFixed(4)}</span></span>
        <span>Worst: <span class="hl" style="color:var(--red)">${s.worst_trade.toFixed(4)}</span></span>
      </div>
      <div class="port-bar-wrap"><div class="port-bar-fill" style="width:${barPct}%;background:${barCol}"></div></div>
    </div>`;
  }).join('');
  return `<div class="section-title"><span>Exit Strategy Comparison</span><span style="color:var(--dim)">${data.total_trades} trades · ${data.bet_sol} SOL/trade</span></div>${header}${rows}`;
}
...
function renderMarket(d) {
  ...
  const barPct = d.score ?? 0;
  const solLine = d.sol?.price ? `<div style="text-align:center;font-size:11px;color:var(--dim);margin-bottom:16px">SOL <span style="color:var(--bright)">$${(+d.sol.price).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</span> <span style="color:${d.sol.change_24h>=0?'var(--green)':'var(--red)'}">${d.sol.change_24h>=0?'+':''}${d.sol.change_24h}% 24h</span></div>` : '';
  const header = `
  <div class="market-score-big" style="color:${col}">${d.score}</div>
  <div class="market-rec" style="color:${col}">${d.recommendation}</div>
  ${solLine}
  <div class="market-bar-wrap"><div class="market-bar-fill" style="width:${barPct}%;background:${col}"></div></div>`;
  const cards = (d.signals||[]).map(s=>`
  <div class="signal-card ${s.status}">
    <div class="sig-name">${s.name} <span class="sig-pts">${s.pts}/${s.max}</span></div>
    <div class="sig-value">${s.value}</div>
    <div class="sig-detail">${s.detail}</div>
  </div>`).join('');
  return `<div class="section-title"><span>Market Conditions</span><span style="color:var(--dim)">${new Date(d.timestamp*1000).toLocaleTimeString()}</span></div>${header}<div class="market-grid">${cards}</div>`;
}
